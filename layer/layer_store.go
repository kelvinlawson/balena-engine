...
func newStoreFromGraphDriver(root string, driver graphdriver.Driver, os string) (Store, error) {
	...
	go func() {
		report := ls.prune(txData)
		if len(report.RemovedDriverLayers) > 0 {
			logrus.Infof("Pruned %d unused graph driver layers", len(report.RemovedDriverLayers))
		}
	}()
...
// pruneReport keeps information about deleted data as a result of calling Prune() on a Store.
type pruneReport struct {
	RemovedDriverLayers []string
}

func (ls *layerStore) prune(txData []fileMetadataTxData) *pruneReport {
	...
	return &pruneReport{RemovedDriverLayers: treatedCacheIDs}
}
